...
export class GraphAnalysisService {
  ...
  async analyzeFullGraph(filterInput?: string | GraphFilterOptions): Promise<FullGraphAnalysisResult> {
    ...
    try {
      // Fetch all relationships first (there are far fewer than entities)
      const dbRels = await prisma.relationship.findMany({
        include: {
      ...
      // Collect only the entity IDs that actually appear in relationships
      ...
      // Build entity filter: only connected entities + any user-supplied filters
      const connectedFilter: any = connectedIds.size > 0
        ? { id: { in: Array.from(connectedIds) } }
        : {};

      const mergedEntityWhere: any = Object.keys(entityWhere).length > 0
        ? { AND: [connectedFilter, entityWhere] }
        : connectedFilter;

      const [dbEntities, dbCases] = await Promise.all([
        prisma.entity.findMany({
          where: Object.keys(mergedEntityWhere).length > 0 ? mergedEntityWhere : undefined,
  ...
  async findEntityPath(sourceId: string, targetId: string, maxDepth = 4): Promise<MultiHopPathResult> {
    ...
    try {
      const [dbSource, dbTarget, dbRels] = await Promise.all([
      ...
      // Only load entities that appear in relationships
      const connectedIds = new Set<string>();
      for (const r of dbRels) { connectedIds.add(r.sourceId); connectedIds.add(r.targetId); }
      allEntities = await prisma.entity.findMany({
        where: connectedIds.size > 0 ? { id: { in: Array.from(connectedIds) } } : undefined,
